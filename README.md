...
> 将长视频文稿识别、目标词定位、目标词选择、批量静音和视频导出整合为一套可安装、可授权、可运营的生产流程。
...
| 解决方案 | 本地语音识别、候选定位、文稿/视频/时间轴联动查看、正常语音保护、批量静音与安全导出 |
| 交付形态 | Windows 桌面客户端、端侧智能处理引擎、授权服务、运营后台和版本发布链路 |
| 项目状态 | 已投入真实使用，目前有一家企业正在参与推广验证 |

清影不是对模型接口的简单封装，也不是只展示界面的 Demo。项目从真实业务流程出发，把 AI 推理、音视频处理、目标词选择、账号授权、运营管理和客户端交付组合成一套可以持续使用和迭代的软件系统。

## 低门槛使用体验

清影将语音识别、目标词定位、时间边界处理和音视频导出封装在可视化工作流中。使用者不需要理解 ASR、强制对齐、模型推理或 FFmpeg，也不需要配置命令行和编写脚本；按照界面引导即可完成一段长视频的目标词处理。

```text
导入视频 → 自动识别文稿 → 输入并选择目标词 → 查看全部命中位置 → 一键批量处理与导出
```

- **界面集中**：可搜索文稿、视频预览、候选跳转、倍速播放、时间轴和导出计划位于同一工作台，无需在多个工具之间反复切换。
- **结果可理解**：每个目标词的全部命中都会映射到文稿和视频位置，使用者可以播放上下文，了解选择该目标词后将统一处理的所有位置。
- **过程可感知**：识别和导出等耗时任务提供状态与进度反馈，发生异常时给出可见提示。
- **操作可控**：AI 负责集中发现候选和定位时间区间，用户以目标词为单位决定是否处理并控制最终导出，兼顾处理效率和操作简洁性。

> 当前处理计划以“目标词”为最小选择单位：选择一个目标词后，将处理该词在视频中的全部命中位置；取消该目标词则全部不处理。目前不支持对同一目标词的某一次命中单独勾选或排除。

这套交互面向内容运营、直播团队和后期人员设计。即使没有算法或编程背景，首次使用者也能从界面理解下一步操作；复杂的端侧模型调度、候选聚合和媒体处理由软件在后台完成。

## 典型应用场景

清影支持处理用户自行指定的任意目标词，不依赖固定敏感词库。用户输入需要处理的平台名称、营销用语、脏话、口误或其他指定词语后，系统会集中定位它们在长视频中的全部候选位置；用户选择需要处理的目标词后，系统批量处理该词对应的所有音频区间。
...
> 清影负责执行用户明确指定的目标词定位与音频处理，不自动判断全部违规、敏感或隐私内容，也不替代平台规则判断和法律审核。对于长期反复使用、高要求交付或风险敏感的素材，建议在发布前进行抽样或最终复查。
...
- 建立从视频导入、文稿识别、目标词搜索、按词选择到批量导出的完整闭环。
...
清影已经在多批次、不同类型的真实长视频素材中持续运行验证，当前核心处理流程和交付链路已具备稳定使用条件。以下选取其中一个具有代表性的实际样例，用于直观说明任务规模、处理效率和最终效果。

在 RTX 3060 实际处理样例中，一段 1 小时的中文口播视频包含 8 类目标词、319 个待处理位置，从识别结果进入批量处理到完成导出约需 10 分钟；导出结果经剪映文稿复查，该样例未发现目标词遗漏。
...
清影将目标词检索、命中位置查看和批量音频处理前置为独立任务，并提供任务进度与缓存管理。常规场景下，处理结果可以直接使用；对于计划长期反复使用、高要求交付或风险敏感的素材，可以再导入专业剪辑软件进行抽样或最终复查。这不仅减少逐处人工操作，也降低了长时间停留在大型剪辑工程中所带来的性能波动和返工风险。上述 **3–8 倍**仅按理想人工耗时计算，未将软件卡顿、重新识别或进度丢失造成的额外时间计入。

> 以上是从多批次真实验证中选取的代表性软件样例，人工耗时部分属于经验估算，不构成固定性能、效率或准确率承诺。实际耗时和结果会受到操作者熟练度、剪辑软件状态、设备性能、显卡、素材编码、语速、背景噪声、目标词数量与模型状态影响。
...
- 设计“AI 定位全部命中、用户按目标词选择、系统统一安全执行”的人机协作方式，降低逐片段操作成本并保留处理控制权。
...
| 批量处理容易误伤正常语音 | 在处理前后执行目标覆盖、非目标词重叠和正常音频保护检查；高风险区间无法通过安全复核时停止自动导出并提示异常，避免未经验证的结果直接交付 |
...
3. **搜索目标词**：检索目标词并汇总该词在全文中的全部命中位置。
4. **查看命中上下文**：通过文稿高亮、视频预览、倍速播放和时间轴了解各处命中。
5. **生成处理计划**：以目标词为单位选择是否处理；选中后处理该词的全部命中位置，并可按需开启时间表达式处理。
6. **安全处理与导出**：批量处理目标词对应音频区间，保护正常语音，并输出可复查的视频。
7. **高要求场景复查（可选）**：对于计划长期反复使用、高要求交付或风险敏感的素材，可将结果导入剪映等工具，对重点位置进行抽样或最终确认。
...
- **三位一体查看**：联动文稿高亮、视频预览和命中时间轴，集中了解目标词的全部出现位置。
- **按词管理计划**：目标词按组管理；选择后统一处理该词的全部命中位置，取消后全部撤回。
